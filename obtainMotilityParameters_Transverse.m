...
% SCALING OPTIONS (in order of priority):
% 1. usePercentileScale - robust scaling based on data percentiles (recommended)
% 2. iWantToUseMyOwnScale - manual fixed values
% 3. If both are false, MATLAB auto-scales each plot independently

usePercentileScale = true;      % Use percentile-based scaling to handle outlier spikes
QSTMapPercentiles = [1, 99];    % [lower, upper] percentiles for QSTMap scaling
xCorrPercentiles = [2, 98];     % [lower, upper] percentiles for XCorr scaling
useSharedQSTMapScale = true;    % If true, both QSTMaps use the same scale for comparison

iWantToUseMyOwnScale = false;   % Manual override with fixed values below
...
convertAmplitudeToUmUnits = 2/size(gutMeshVelsPCoords, 4);  % FFT normalization only (data already in um/s)
...
%% Compute surface values for both QSTMaps first (needed for shared scaling)

% Longitudinal QSTMap values
...
if iWantToUseMyOwnScale
    QSTMapScaleL = [QSTMapMin, QSTMapMax];
    QSTMapScaleT = [QSTMapMin, QSTMapMax];
elseif usePercentileScale
    if useSharedQSTMapScale
        % Combine both datasets to get shared scale
        allQSTData = [surfaceValuesL(:); surfaceValuesT(:)];
        sharedMin = prctile(allQSTData, QSTMapPercentiles(1));
        sharedMax = prctile(allQSTData, QSTMapPercentiles(2));
        QSTMapScaleL = [sharedMin, sharedMax];
        QSTMapScaleT = [sharedMin, sharedMax];
    else
        % Independent scaling for each
        QSTMapScaleL = [prctile(surfaceValuesL(:), QSTMapPercentiles(1)), ...
                        prctile(surfaceValuesL(:), QSTMapPercentiles(2))];
        QSTMapScaleT = [prctile(surfaceValuesT(:), QSTMapPercentiles(1)), ...
                        prctile(surfaceValuesT(:), QSTMapPercentiles(2))];
    end
else
    QSTMapScaleL = [];
    QSTMapScaleT = [];
...
% Display surface plot - TOP LEFT (position 1)
subplot(2, 3, 1);
...
% Display surface plot - BOTTOM LEFT (position 4)
subplot(2, 3, 4);
...
% Determine XCorr scaling
if iWantToUseMyOwnScale
    xCorrScale = [xCorrMin, xCorrMax];
elseif usePercentileScale
    xCorrScale = [prctile(trueXCorr(:), xCorrPercentiles(1)), ...
                  prctile(trueXCorr(:), xCorrPercentiles(2))];
else
    xCorrScale = [];
end
